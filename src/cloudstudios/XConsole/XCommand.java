...
public class XCommand {
	...
	public XCommand(boolean read, String cmd){
		try{
			packed = false;
	...
	public XCommand(boolean read, String cmd, int device){
		this(read,cmd);
		setDevice(device);
	}
	
	public XCommand(boolean read, String cmd,int device, boolean input, int channel){
		this(read,cmd);
		setDevice(device);
		setChannel(input,channel);
	}
	
	public XCommand(boolean read, String cmd, int device, boolean input, int channel, int data){
		this(read,cmd,device,input,channel);
		setData(data);
	}
	
	public XCommand(boolean read, String cmd, int device, int data){
		this(read,cmd);
		setData(data);
	}
	public XCommand(String cmd, int data){
		this(false,cmd);
		setData(data);
	}

	
	public XCommand(Socket s){
		bytes = new ArrayList<Byte>();
